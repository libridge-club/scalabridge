...
@Test
class BiddingBoxTest extends AnyFlatSpec {
    ...
    "A BiddingBox" should "get bid calls from label" in {
        assertResult(Bid(OddTricks.THREE,Strain.NOTRUMPS))(BiddingBox.getOption(THREE_NO_TRUMPS_STRING).get)
    }
    "A BiddingBox" should "get bid calls from oddTricks and Strain" in {
        assertResult(Bid(OddTricks.THREE,Strain.NOTRUMPS))(BiddingBox.getBid(OddTricks.THREE,Strain.NOTRUMPS))
    }
}
