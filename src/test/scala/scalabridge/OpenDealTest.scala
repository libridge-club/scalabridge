...
class OpenDealTest extends AnyFlatSpec {
  ...
  "An OpenDeal " should "return its deal actions in first to last order" in {
    ...
    val listOfCardStrings = List("h2", "h4", "hJ", "hA", "s5", "s3", "sA", "sJ", "d2", "d4", "dQ", "dA", "h8", "hK", "h9", "c4", "d5", "dK", "d9", "dT", "d3", "dJ", "s4", "d6", "sQ", "sK", "h3", "s6", "d8", "c2", "c6", "sT", "cJ", "c3", "cA", "c9")
    val listOfCards = listOfCardStrings.map((cardString: String) => new Card(getSuit(cardString), getRank(cardString)))
