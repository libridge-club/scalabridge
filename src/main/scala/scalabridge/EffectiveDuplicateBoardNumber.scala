...
object EffectiveDuplicateBoardNumber:
  ...
  private val FIRST_DUPLICATE_BOARD_NUMBER =
    EffectiveDuplicateBoardNumber.values(0)
  private val LAST_DUPLICATE_BOARD_NUMBER =
    EffectiveDuplicateBoardNumber.values(COMPLETE_CYCLE_SIZE - 1)
  ...
  private def getNext(
      effectiveDuplicateBoardNumber: EffectiveDuplicateBoardNumber
  ) = {
    if (LAST_DUPLICATE_BOARD_NUMBER == effectiveDuplicateBoardNumber)
      FIRST_DUPLICATE_BOARD_NUMBER
    else
      EffectiveDuplicateBoardNumber.fromOrdinal(
        effectiveDuplicateBoardNumber.ordinal + 1
      )
  }
end EffectiveDuplicateBoardNumber
